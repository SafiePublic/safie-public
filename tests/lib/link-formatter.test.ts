...
describe('formatBasicLink', () => {
  it('returns html anchor and plain record name', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com">My Record</a>');
    expect(result.plain).toBe('My Record');
  });

  it('escapes HTML in record name for html output', () => {
    const result = formatBasicLink('<script>alert("xss")</script>', 'https://example.com');
  ...
  it('escapes special characters in url for html output', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com/r?a=1&amp;b=&quot;2&quot;">Record</a>');
  });
});

describe('formatExtendedLink', () => {
  it('links name only by default (showLabel=false)', () => {
    const result = formatExtendedLink('Record', 'https://example.com', 'Code', 'ABC-001', false);
    expect(result.html).toBe('<a href="https://example.com">Record</a>(ABC-001)');
    expect(result.plain).toBe('Record(ABC-001)');
  });

  it('links name only by default (showLabel=true)', () => {
    const result = formatExtendedLink('Record', 'https://example.com', 'Code', 'ABC-001', true);
    expect(result.html).toBe('<a href="https://example.com">Record</a>(Code:ABC-001)');
    expect(result.plain).toBe('Record(Code:ABC-001)');
  });

  it('escapes HTML in name-only mode', () => {
    const result = formatExtendedLink('R&D', 'https://example.com', 'Label', 'Val<1>"', true);
    expect(result.html).toBe('<a href="https://example.com">R&amp;D</a>(Label:Val&lt;1&gt;&quot;)');
  ...
  it('links entire text when linkNameOnly=false', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com">Record(ABC-001)</a>');
    expect(result.plain).toBe('Record(ABC-001)');
  });
  ...
  it('links entire text with label when linkNameOnly=false', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com">Record(Code:ABC-001)</a>');
    expect(result.plain).toBe('Record(Code:ABC-001)');
  });
});
...
describe('extractFieldLabels', () => {
  ...
  it('excludes builtin variables', () => {
    expect(extractFieldLabels('${object}: ${name}')).toEqual([]);
  });

  it('extracts multiple field labels', () => {
    expect(extractFieldLabels('${name}(${商品コード} / ${カテゴリ})')).toEqual([
      '商品コード',
      'カテゴリ',
    ]);
  });
  ...
  it('deduplicates labels', () => {
    expect(extractFieldLabels('${商品コード} - ${商品コード}')).toEqual(['商品コード']);
  });
...
describe('formatTemplateLink', () => {
  ...
  it('links name only by default when ${name} present', () => {
    const result = formatTemplateLink('Product A', url, '${name}', {}, '商品');
  ...
  it('links name only with a single field', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com">Product A</a>(ABC-001)');
  ...
  it('expands ${object}', () => {
    const result = formatTemplateLink('Product A', url, '${name} - ${object}', {}, '商品');
  ...
  it('replaces unknown variables with empty string', () => {
    const result = formatTemplateLink('Rec', url, '${name}(${unknown})', {}, 'Obj');
  ...
  it('links entire text when linkNameOnly=false', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com">Product A(ABC-001)</a>');
    expect(result.plain).toBe('Product A(ABC-001)');
  });
  ...
  it('links entire text when no ${name} in format (even if linkNameOnly=true)', () => {
    ...
    expect(result.html).toBe('<a href="https://example.com">商品: ABC-001</a>');
    expect(result.plain).toBe('商品: ABC-001');
  });

  it('handles multiple ${name} occurrences', () => {
    const result = formatTemplateLink('Rec', url, '${name} - ${name}', {}, 'Obj');
    expect(result.html).toBe(
...
describe('appendToastMessages', () => {
  ...
  it('escapes HTML special characters in toast messages', () => {
    ...
    expect(result.html).toContain('⚠ エラー: 値が &lt;script&gt; &amp; &quot;invalid&quot;');
  });
});
